...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        public virtual void DeleteCategory(Category category)
        {
            ...
            var subcategories = GetAllCategoriesByParentCategoryId(category.CategoryId, true);
            foreach (var subcategory in subcategories)
            {
                subcategory.ParentCategoryId = string.Empty;
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            if (!showHidden)
                query = query.Where(c => c.CategoryStatus == 1);
        ...
        {
            var key = string.Format(CATEGORIES_BY_PARENT_CATEGORY_ID_KEY, parentCategoryId, showHidden, _workContext.CurrentCustomer.Id, 0, includeAllLevels);
            return _cacheManager.Get(key, () =>
            {
                ...
                {
                    childCategories.AddRange(GetAllCategoriesByParentCategoryId(category.CategoryId, showHidden, true));
                }
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            var key = string.Format(PRODUCTCATEGORIES_ALLBYCATEGORYID_KEY, showHidden, categoryId, pageIndex, pageSize, _workContext.CurrentCustomer.Id, 0);
            return _cacheManager.Get(key, () =>
            {
                ...
                if (!showHidden && (!_catalogSettings.IgnoreAcl || !_catalogSettings.IgnoreStoreLimitations))
                {
                    if (!_catalogSettings.IgnoreAcl)
                    {
                        ...
                        query = from pc in query
                                join c in _categoryRepository.Table on pc.CategoryId equals c.CategoryId
                                join acl in _aclRepository.Table
                                on new { c1 = c.Id, c2 = "Category" } equals new { c1 = acl.EntityId, c2 = acl.EntityName } into c_acl
                                from acl in c_acl.DefaultIfEmpty()
                                where !c.SubjectToAcl || allowedCustomerRolesIds.Contains(acl.CustomerRoleId)
        ...
        {
            return GetProductCategoriesByProductId(productId, 0, showHidden);
        }
        ...
        public virtual string[] GetNotExistingCategories(string[] categoryNames)
        {
            ...
            var filter = query.Select(c => c.CategoryName).Where(c => queryFilter.Contains(c)).ToList();
        ...
        public virtual IDictionary<int, string[]> GetProductCategoryIds(int[] productIds)
        {
        ...
        /// <returns>Categories</returns>
        public virtual List<Category> GetCategoriesByIds(string[] categoryIds)
        {
            if (categoryIds == null || categoryIds.Length == 0)
                return new List<Category>();

            var query = from p in _categoryRepository.Table
                where categoryIds.Contains(p.CategoryId) && !(p.CategoryStatus == -1)
                select p;
