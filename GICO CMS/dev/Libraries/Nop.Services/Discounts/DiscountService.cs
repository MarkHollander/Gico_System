...
namespace Nop.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
            ...
            RequirementGroupInteractionType groupInteractionType, Customer customer, List<string> errors)
        {
            ...
            foreach (var requirement in requirements)
            {
                ...
                else
                {
                    ...
                    if (!_pluginFinder.AuthenticateStore(requirementRulePlugin.PluginDescriptor, 0))
                        continue;
        ...
        public virtual IList<string> GetAppliedCategoryIds(DiscountForCaching discount, Customer customer)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));
                ...
                case DiscountLimitationType.NTimesPerCustomer:
                    {
                        ...
                        {
                            var usedTimes = GetAllDiscountUsageHistory(discount.Id, customer.CustomerId, null, 0, 1).TotalCount;
                            if (usedTimes >= discount.LimitationTimes)
                            {
        ...
        public IPagedList<DiscountUsageHistory> GetAllDiscountUsageHistory(int? discountId = null, string customerId = null, int? orderId = null,
            int pageIndex = 0, int pageSize = Int32.MaxValue)
        {
            ...
            if (!string.IsNullOrEmpty(customerId))
                discountUsageHistory = discountUsageHistory.Where(historyRecord => historyRecord.Order != null && historyRecord.Order.CustomerId == customerId);
