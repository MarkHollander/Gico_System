    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        protected virtual int DeleteGuestCustomersUseLinq(DateTime? createdFromUtc, DateTime? createdToUtc, bool onlyWithoutShoppingCart)
        {
            ...
            if (createdFromUtc.HasValue)
                query = query.Where(c => createdFromUtc.Value <= c.CreatedDateUtc);
            if (createdToUtc.HasValue)
                query = query.Where(c => createdToUtc.Value >= c.CreatedDateUtc);
            ...
            query = from c in query
                    join o in _orderRepository.Table on c.CustomerId equals o.CustomerId into c_o
                    from o in c_o.DefaultIfEmpty()
                    where !c_o.Any()
                    select c;
            //no blog comments
            query = from c in query
                    join bc in _blogCommentRepository.Table on c.CustomerId equals bc.CustomerId into c_bc
                    from bc in c_bc.DefaultIfEmpty()
                    where !c_bc.Any()
                    select c;
            //no news comments
            query = from c in query
                    join nc in _newsCommentRepository.Table on c.CustomerId equals nc.CustomerId into c_nc
                    from nc in c_nc.DefaultIfEmpty()
                    where !c_nc.Any()
                    select c;
            //no product reviews
            query = from c in query
                    join pr in _productReviewRepository.Table on c.CustomerId equals pr.CustomerId into c_pr
                    from pr in c_pr.DefaultIfEmpty()
                    where !c_pr.Any()
                    select c;
            //no product reviews helpfulness
            query = from c in query
                    join prh in _productReviewHelpfulnessRepository.Table on c.CustomerId equals prh.CustomerId into c_prh
                    from prh in c_prh.DefaultIfEmpty()
                    where !c_prh.Any()
                    select c;
            //no poll voting
            query = from c in query
                    join pvr in _pollVotingRecordRepository.Table on c.CustomerId equals pvr.CustomerId into c_pvr
                    from pvr in c_pvr.DefaultIfEmpty()
                    where !c_pvr.Any()
                    select c;
            //no forum posts 
            query = from c in query
                    join fp in _forumPostRepository.Table on c.CustomerId equals fp.CustomerId into c_fp
                    from fp in c_fp.DefaultIfEmpty()
                    where !c_fp.Any()
                    select c;
            //no forum topics
            query = from c in query
                    join ft in _forumTopicRepository.Table on c.CustomerId equals ft.CustomerId into c_ft
        ...
        public virtual IPagedList<Customer> GetAllCustomers(DateTime? createdFromUtc = null,
            DateTime? createdToUtc = null, int affiliateId = 0, string vendorId = "",
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            {
                query = query
                    .Join(_gaRepository.Table, x => x.CustomerId, y => y.EntityId.ToString(), (x, y) => new { Customer = x, Attribute = y })
        ...
        //public virtual IList<CustomerPassword> GetCustomerPasswords(string customerId = "",
        ...
        //    //filter by customer
        //    if (!string.IsNullOrEmpty(customerId))
        //        query = query.Where(password => password.CustomerId == customerId);
        ...
        //public virtual CustomerPassword GetCurrentPassword(string customerId)
        //{
        //    if (string.IsNullOrEmpty(customerId))
        //        return null;
