...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        protected bool PasswordsMatch(string customerPassword, string enteredPassword, string customerCode, string salt)
        {
            if (customerPassword == null || string.IsNullOrEmpty(enteredPassword))
                return false;
            string passwordHash = _encryptionService.Encryption(customerCode, enteredPassword, salt);
            return customerPassword.Equals(passwordHash);
        }
        ...
        public virtual CustomerLoginResults ValidateCustomer(string usernameOrEmail, string password)
        {
            ...
            if (!PasswordsMatch(customer.Password, password, customer.Code, customer.PasswordSalt))
            {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            request.Customer.FullName = request.Customer.FullName;
            request.Customer.PhoneNumber = request.Customer.PhoneNumber;
            string passwordHash = _encryptionService.Encryption(string.Empty, request.Password, out string salt);
            request.Customer.PasswordSalt = salt;
            request.Customer.Password = passwordHash;
            ...
            {
                request.Customer.CustomerId = Guid.NewGuid().ToString("N");
                request.Customer.EmailToRevalidate = string.Empty;
                request.Customer.AdminComment = string.Empty;
                request.Customer.LastIpAddress = string.Empty;
                request.Customer.BillingAddressId = string.Empty;
                request.Customer.ShippingAddressId = string.Empty;
                request.Customer.Code = string.Empty;
                request.Customer.Version = 1;
                request.Customer.LanguageId = string.Empty;
                request.Customer.CreatedUid = string.Empty;
                request.Customer.UpdatedUid = string.Empty;
                request.Customer.CreatedDateUtc = DateTime.UtcNow;
                request.Customer.UpdatedDateUtc = DateTime.UtcNow;
