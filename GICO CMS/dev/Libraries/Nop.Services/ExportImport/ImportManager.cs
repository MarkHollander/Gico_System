    ...
    public partial class ImportManager : IImportManager
    {
        ...
        protected virtual string UpdateCategoryByXlsx(Category category, PropertyManager<Category> manager, Dictionary<string, Category> allCategories, bool isNew, out bool isParentCategoryExists)
        {
            ...
            foreach (var property in manager.GetProperties)
            {
                ...
                {
                    case "Name":
                        category.CategoryName = property.StringValue.Split(new[] { ">>" }, StringSplitOptions.RemoveEmptyEntries).Last().Trim();
                        break;
                    case "Description":
                        category.CategoryDescription = property.StringValue;
                        break;                    
                    case "ParentCategoryId":
                        if (!isParentCategorySet)
                        {
                            ...
                            category.ParentCategoryId = parentCategory?.CategoryId ?? string.Empty;
                        }
                        break;
                        ...
                        if (_catalogSettings.ExportImportCategoriesUsingCategoryName && !isParentCategorySet)
                        {
                            ...
                            if (!string.IsNullOrEmpty(categoryName))
                            {
                                var parentCategory = allCategories.ContainsKey(categoryName)
                                    ...
                                    : allCategories.Values.FirstOrDefault(c => c.CategoryName.Equals(categoryName, StringComparison.InvariantCulture));
        ...
        protected virtual void SaveCategory(bool isNew, Category category, Dictionary<string, Category> allCategories, string curentCategoryBreadCrumb, bool setSeName, string seName)
        {
            ...
            //search engine name
            if (setSeName)
                _urlRecordService.SaveSlug(category, category.ValidateSeName(seName, category.CategoryName, true), 0);
        }
        ...
        public virtual void ImportProductsFromXlsx(Stream stream)
        {
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                manager.SetSelectList("Vendor", _vendorService.GetAllVendors(showHidden: true).Select(v => v as BaseEntity).ToSelectList(p => (p as Vendor)?.VendorName ?? string.Empty));
                ...
                for (var iRow = 2; iRow < endRow; iRow++)
                {
                    ...
                    { 
                        ...
                        //category mappings
                        var categories = isNew || !allProductsCategoryIds.ContainsKey(product.Id) ? new string[0] : allProductsCategoryIds[product.Id];
                        var importedCategories = categoryNames.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries).Select(x => allCategories.ContainsKey(x.Trim()) ? allCategories[x.Trim()].CategoryId : allCategories.Values.First(c => c.CategoryName == x.Trim()).CategoryId).ToList();
