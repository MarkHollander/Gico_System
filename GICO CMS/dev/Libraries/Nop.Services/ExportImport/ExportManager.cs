    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual void WriteCategories(XmlWriter xmlWriter, string parentCategoryId)
        {
            ...
            foreach (var category in categories)
            {
                ...
                xmlWriter.WriteString("Name", category.CategoryName);
                xmlWriter.WriteString("Description", category.CategoryDescription);
                ...
                var productCategories = _categoryService.GetProductCategoriesByCategoryId(category.CategoryId, showHidden: true);
        ...
        protected virtual string GetCategories(Product product)
        {
            ...
            {
                categoryNames += _catalogSettings.ExportImportProductCategoryBreadcrumb ? pc.Category.GetFormattedBreadCrumb(_categoryService) : pc.Category.CategoryName;
                categoryNames += ";";
            }
        ...
        public virtual string ExportCategoriesToXml()
        {
            ...
            xmlWriter.WriteAttributeString("Version", NopVersion.CurrentVersion);
            WriteCategories(xmlWriter, string.Empty);
            xmlWriter.WriteEndElement();
            xmlWriter.WriteEndDocument();
            xmlWriter.Close();
            return stringWriter.ToString();
        ...
        public virtual byte[] ExportCategoriesToXlsx(IList<Category> categories)
        {
            ...
            if (_catalogSettings.ExportImportCategoriesUsingCategoryName)
            {
                ...
                parentCatagories = _categoryService.GetCategoriesByIds(categories.Select(c => c.ParentCategoryId).Where(id => !string.IsNullOrEmpty(id)).ToArray());
            }
            ...
            var properties = new[]
            {
                ...
                new PropertyByName<Category>("Name", p => p.CategoryName),
                new PropertyByName<Category>("Description", p => p.CategoryDescription),                
                ...
                new PropertyByName<Category>("ParentCategoryName", p => parentCatagories.FirstOrDefault(c => c.CategoryId == p.ParentCategoryId)?.GetFormattedBreadCrumb(_categoryService), !_catalogSettings.ExportImportCategoriesUsingCategoryName),
        ...
        public virtual byte[] ExportProductsToXlsx(IEnumerable<Product> products)
        {
            var properties = new[]
            {
                ...
                {
                    DropDownElements = _vendorService.GetAllVendors(showHidden: true).Select(v => v as BaseEntity).ToSelectList(p => (p as Vendor)?.VendorName ?? string.Empty),
                    AllowBlank = true
                },
        ...
        public virtual byte[] ExportCustomersToXlsx(IList<Customer> customers)
        {
            ...
            var properties = new[]
            {
                ...
                //new PropertyByName<Customer>("Password", p => _customerService.GetCurrentPassword(p.CustomerId)?.Password),
                //new PropertyByName<Customer>("PasswordFormatId", p => _customerService.GetCurrentPassword(p.CustomerId)?.PasswordFormatId ?? 0),
                //new PropertyByName<Customer>("PasswordSalt", p => _customerService.GetCurrentPassword(p.CustomerId)?.PasswordSalt),
                ...
                new PropertyByName<Customer>("CreatedOnUtc", p => p.CreatedDateUtc),
                //attributes
        ...
        public virtual string ExportCustomersToXml(IList<Customer> customers)
        {
            ...
            foreach (var customer in customers)
            {
               ...
               // xmlWriter.WriteElementString("Username", null, customer.Username);
                ...
                //var customerPassword = _customerService.GetCurrentPassword(customer.CustomerId);
                ...
                xmlWriter.WriteElementString("CreatedOnUtc", null, customer.CreatedDateUtc.ToString());
