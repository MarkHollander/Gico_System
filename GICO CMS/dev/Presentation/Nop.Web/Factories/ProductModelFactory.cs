...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual ProductReviewOverviewModel PrepareProductReviewOverviewModel(Product product)
        {
            ...
            {
                var cacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_REVIEWS_MODEL_KEY, product.Id, 0);
                ...
                productReview = _cacheManager.Get(cacheKey, () =>
                {
                    return new ProductReviewOverviewModel
                    {
                        RatingSum = product.ProductReviews
                                .Where(pr => pr.IsApproved && pr.StoreId == 0)
                                .Sum(pr => pr.Rating),
                        TotalReviews = product
                                .ProductReviews
                                .Count(pr => pr.IsApproved && pr.StoreId == 0)
                    };
                });
            }
            else
        ...
        protected virtual IList<ProductTagModel> PrepareProductTagModels(Product product)
        {
            ...
            var productTagsCacheKey = string.Format(ModelCacheEventConsumer.PRODUCTTAG_BY_PRODUCT_MODEL_KEY, product.Id, _workContext.WorkingLanguage.Id, 0);
        ...
        protected virtual PictureModel PrepareProductDetailsPictureModel(Product product, bool isAssociatedProduct, out IList<PictureModel> allPictureModels)
        {
            ...
            var productPicturesCacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_DETAILS_PICTURES_MODEL_KEY, product.Id, defaultPictureSize, isAssociatedProduct, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), 0);
        ...
        //        if (!isAssociatedProduct)
        //        {
        //            var associatedProducts = _productService.GetAssociatedProducts(product.Id);//0);
        //            foreach (var associatedProduct in associatedProducts)
        ...
        public virtual ProductReviewsModel PrepareProductReviewsModel(ProductReviewsModel model, Product product)
        {
            ...
            var productReviews = _catalogSettings.ShowProductReviewsPerStore
                ? product.ProductReviews.Where(pr => pr.IsApproved //&& pr.StoreId == 0
        ...
        public virtual CustomerProductReviewsModel PrepareCustomerProductReviewsModel(int? page)
        {
            ...
            var list = _productService.GetAllProductReviews(customerId: _workContext.CurrentCustomer.CustomerId, 
                approved: null, 
                pageIndex: pageIndex, 
                pageSize: pageSize);
