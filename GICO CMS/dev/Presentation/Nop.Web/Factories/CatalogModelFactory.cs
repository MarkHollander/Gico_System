...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        protected virtual List<string> GetChildCategoryIds(string parentCategoryId)
        {
            ...
            return _cacheManager.Get(cacheKey, () =>
            {
                ...
                foreach (var category in categories)
                {
                    ...
                    categoriesIds.AddRange(GetChildCategoryIds(category.CategoryId));
                }
        ...
        public virtual CategoryModel PrepareCategoryModel(Category category, CatalogPagingFilteringModel command)
        {
            ...
            {
                Id = category.Id,
                Name = category.GetLocalized(x => x.CategoryName),
                Description = category.GetLocalized(x => x.CategoryDescription),
            ...
            if (_catalogSettings.CategoryBreadcrumbEnabled)
            {
                ...
                model.CategoryBreadcrumb = _cacheManager.Get(breadcrumbCacheKey, () =>
                    ...
                    {
                        Id = catBr.Id,
                        Name = catBr.GetLocalized(x => x.CategoryName),
            ...
            model.SubCategories = _cacheManager.Get(subCategoriesCacheKey, () =>
                _categoryService.GetAllCategoriesByParentCategoryId(category.CategoryId)
                .Select(x =>
                {
                    var subCatModel = new CategoryModel.SubCategoryModel
                    {
                        Id = x.Id,
                        Name = x.GetLocalized(y => y.CategoryName),
                        SeName = x.GetSeName(),
                        Description = x.GetLocalized(y => y.CategoryDescription)
                    };

                    //prepare picture model
                    var categoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, x.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), 0);
                    subCatModel.PictureModel = _cacheManager.Get(categoryPictureCacheKey, () =>
                    {
                        //var picture = _pictureService.GetPictureById(x.PictureId);
                        var picture = _pictureService.GetPictureById(0);
            ...
            if (!_catalogSettings.IgnoreFeaturedProducts)
            {
                ...
                var cacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_HAS_FEATURED_PRODUCTS_KEY, category.Id,
                    string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()), 0);
                ...
                if (!hasFeaturedProductsCache.HasValue)
                {
                    ...
                    featuredProducts = _productService.SearchProducts(
                       categoryIds: new List<string> { category.CategoryId },
                       storeId: 0,
            ...
            if (_catalogSettings.ShowProductsFromSubcategories)
            {
                ...
                categoryIds.AddRange(GetChildCategoryIds(category.CategoryId));
            }
            //products
        ...
        public virtual CategoryNavigationModel PrepareCategoryNavigationModel(string currentCategoryId, int currentProductId)
        {
            //get active category
            var activeCategoryId = string.Empty;
        ...
        public virtual List<CategoryModel> PrepareHomepageCategoryModels()
        {
            ...
            var model = _cacheManager.Get(categoriesCacheKey, () =>
                ...
                .Select(category =>
                {
                    ...
                    //prepare picture model
                    //var categoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), 0);
            ...
            bool loadSubCategories = true, IList<Category> allCategories = null)
        {
            ...
            if (allCategories == null)
            {
                ...
                allCategories = _categoryService.GetAllCategories(storeId: 0);
            }
                ...
                if (_catalogSettings.ShowCategoryProductNumber)
                {
                    ...
                    categoryModel.NumberOfProducts = _cacheManager.Get(cacheKey, () =>
                    {
                        ...
                        if (_catalogSettings.ShowCategoryProductNumberIncludingSubcategories)
                            categoryIds.AddRange(GetChildCategoryIds(category.CategoryId));
                        return _productService.GetNumberOfProductsInCategory(categoryIds, 0);
                    });
                }

                if (loadSubCategories)
                {
                    var subCategories = PrepareCategorySimpleModels(category.CategoryId, loadSubCategories, allCategories);
        ...
        public virtual List<ManufacturerModel> PrepareManufacturerAllModels()
        {
            ...
            var manufacturers = _manufacturerService.GetAllManufacturers(storeId: 0);
            foreach (var manufacturer in manufacturers)
            {
                ...
                var manufacturerPictureCacheKey = string.Format(ModelCacheEventConsumer.MANUFACTURER_PICTURE_MODEL_KEY, manufacturer.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), 0);
        ...
        public virtual VendorModel PrepareVendorModel(Vendor vendor, CatalogPagingFilteringModel command)
        {
            ...
            {
                Id = vendor.Id,
                Name = vendor.GetLocalized(x => x.VendorName),
                Description = vendor.GetLocalized(x => x.VendorDescription),
        ...
        public virtual List<VendorModel> PrepareVendorAllModels()
        {
            ...
            foreach (var vendor in vendors)
            {
                ...
                var pictureCacheKey = string.Format(ModelCacheEventConsumer.VENDOR_PICTURE_MODEL_KEY, vendor.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), 0);
        ...
        {
            var cacheKey = string.Format(ModelCacheEventConsumer.PRODUCTTAG_POPULAR_MODEL_KEY, _workContext.WorkingLanguage.Id, 0);
            var cachedModel = _cacheManager.Get(cacheKey, () =>
            {
                ...
                var allTags = _productTagService
                    ...
                    .Where(x => _productTagService.GetProductCount(x.Id, 0) > 0)
        ...
        public virtual SearchModel PrepareSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            var categories = _cacheManager.Get(cacheKey, () =>
            {
                ...
                //all categories
                var allCategories = _categoryService.GetAllCategories(storeId: 0);
                foreach (var c in allCategories)
                {
                    ...
                    {
                        categoryBreadcrumb += breadcrumb[i].GetLocalized(x => x.CategoryName);
            ...
            if (categories.Any())
            {
                ...
                foreach (var c in categories)
                {
                    model.AvailableCategories.Add(new SelectListItem
                    {
                        Value = c.Id.ToString(),
                        Text = c.Breadcrumb,
                        Selected = model.cid == c.CategoryId
                    });
                }
            }
                ...
                if (vendors.Any())
                {
                    ...
                    foreach (var vendor in vendors)
                        model.AvailableVendors.Add(new SelectListItem
                        {
                            ...
                            Text = vendor.GetLocalized(x => x.VendorName),
            ...
            if (isSearchTermSpecified)
            {
                ...
                else
                {
                    ...
                    {
                        var searchTerm = _searchTermService.GetSearchTermByKeyword(searchTerms, 0);
                        if (searchTerm != null)
                        {
                            searchTerm.Count++;
