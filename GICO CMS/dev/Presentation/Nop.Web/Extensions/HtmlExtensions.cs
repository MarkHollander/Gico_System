...
namespace Nop.Web.Extensions
{
    public static class HtmlExtensions
    {
        ...
        public static string GetTopicSeName<T>(this IHtmlHelper<T> html, string systemName)
        {
            ...
            var cacheKey = string.Format(ModelCacheEventConsumer.TOPIC_SENAME_BY_SYSTEMNAME, systemName, workContext.WorkingLanguage.Id, 0);
            var cachedSeName = cacheManager.Get(cacheKey, () =>
            {
                var topicService = EngineContext.Current.Resolve<ITopicService>();
                var topic = topicService.GetTopicBySystemName(systemName, 0);
                if (topic == null)
                    return "";
        ...
        public static string GetTopicTitle<T>(this IHtmlHelper<T> html, string systemName)
        {
            ...
            //static cache manager
            var cacheManager = EngineContext.Current.Resolve<IStaticCacheManager>();
            var cacheKey = string.Format(ModelCacheEventConsumer.TOPIC_TITLE_BY_SYSTEMNAME, systemName, workContext.WorkingLanguage.Id, 0);
